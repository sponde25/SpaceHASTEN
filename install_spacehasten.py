...
def _write_ini(path: Path, *, answers: dict[str, str]) -> None:
    ...
    lines.append("TRAIN_NUM_WORKERS = 8")
    lines.append("TRAIN_DEVICES = 1")
    lines.append("TRAIN_MP_HIDDEN_SIZE = 300")
    lines.append("TRAIN_MP_DEPTH = 3")
    lines.append("TRAIN_FFN_HIDDEN_SIZE = 300")
    lines.append("TRAIN_FFN_LAYERS = 1")
    lines.append("TRAIN_DROPOUT = 0.1")
    lines.append("TRAIN_ACTIVATION = relu")
    lines.append("TRAIN_BATCH_NORM = 0")
    lines.append("TRAIN_WARMUP_EPOCHS = 2")
    ...
    lines.append("TRAIN_WARM_START = true")
    lines.append("TRAIN_WARM_EPOCHS = 10")
    lines.append("TRAIN_WARM_NEW_DATA_REPEAT = 2")
    lines.append("TRAIN_WARM_WARMUP_EPOCHS = 1")
    lines.append("TRAIN_WARM_INIT_LR = 3e-5")
    lines.append("TRAIN_WARM_MAX_LR = 3e-4")
    lines.append("TRAIN_WARM_FINAL_LR = 3e-5")
    lines.append("TRAIN_WARM_EARLY_STOPPING_PATIENCE = 10")
    lines.append("TRAIN_PIN_MEMORY = true")
    lines.append("TRAIN_NON_BLOCKING = true")
    lines.append("TRAIN_DEFER_BATCH_METRICS = true")
    lines.append("TRAIN_PERSISTENT_WORKERS = false")
