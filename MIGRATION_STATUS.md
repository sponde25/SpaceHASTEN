...
| 13 | `stages/seeds.py`, `stages/export.py`, `stages/archive.py` | done | | `stages/seeds.import_seeds` (mp.Pool over `core.molecules.tautomer_hash`, dual SMI/CSV input, dock_param/grid blob ingestion + properties write, optional auto dock→train→cluster cascade gated on `auto_train`); `stages/export.export_csv` (legacy `<smilesid>/<spacehastenid>` packing + sorted CSV) and `export_poses` (driving the legacy `export_poses.py` via configurable `$SCHRODINGER/run`); `stages/archive` quartet — `archive_create(bundle=False|True)` produces `.archived-spacehasten[.tgz]` from the single-root layout, `archive_restore`/`archive_extract` invert (refusing non-empty targets), `archive_clean` removes regenerable `simsearch/`/`docking/`/`clustering/` while preserving `.dbsh`/manifest/models/logs. New `Settings.paths.{schrodinger_run,export_poses_script}` fields plumb the Schrödinger toolkit invocation. 14 new tests under `tests/integration/{test_seeds_local,test_export_local,test_archive_local}.py`; 144 tests; ruff/mypy clean. |
| 14 | `cli/main.py` — argparse subcommands | done | | `cli/_common.py` builds `Database`/`WorkDir`/`Scheduler`/`Settings` from the global `--db`/`--config`/`--scheduler`/`--partition`/`--scratch`/`--log-level`/`--json` namespace. `cli/main.main()` wires every Session 8–13 stage behind argparse subcommands: `init`, `import-seeds`, `train`, `predict`, `search`, `dock`, `cluster`, `screen` (legacy macro: optional `--train-first`, then simsearch(docked) → simsearch(predicted)×2 → dock per `--rounds`), `export {csv,poses}`, `archive {create,extract,restore,clean}`, `status`/`resume` (manifest summary, JSON via `--json`), `verify` (placeholder pointing at Session 15). Console script `spacehasten` works after `pip install -e .`. Tests: `tests/unit/test_cli.py` (28 tests covering top-level/subcommand `--help` exit-zero, argparse choice validation, missing-required-arg errors, smoke parsing for every subcommand, and `init` workspace bootstrap); `tests/integration/test_cli_screen_local.py` (2 tests verifying the screen macro ordering invariant via stage monkeypatches). 178 tests; ruff clean; mypy clean on the core/stages/scheduler/workspace strict scope. |
| 15 | Port `verify_spacehasten.py`; cutover | done | | `cli/verify.py` runs the six legacy checks (pigz, scheduler smoke array, clustering, docking, training, biosolveit) via the new stage APIs against an auto-bootstrapped temp workdir; reuses `examples.smi`/`test_dock.in`/`grid-test_dock.zip`/`example.smi`/`example.csv` (resolved by walking up to `pyproject.toml`); checks are individually selectable via `--only`/`--skip` and failures are summarised at the end. Legacy tree (gui.py, spacehasten.py, spacehasten bash launcher, cmdline.py, cfg.py, control.py, chunkpredict.py, model_runner_*.py, *_functions.py, sec_clustering.sh, verify_spacehasten.py, verify) moved to top-level `legacy/` with a README mapping each file to its replacement. `spacehasten-legacy-gui` console script (via `spacehasten._legacy_runner.gui_main`) runs `legacy/gui.py` for one release. `install_spacehasten.py` rewritten: writes `spacehasten.ini`, copies verify fixtures into the install dir, runs `pip install <repo>`. 178 tests pass; ruff clean; mypy clean. **Note:** on-cluster run failed clustering/docking/training/biosolveit; root causes captured in Session 15.1. |
| 15.1 | Verify hardening | not-started | | Stage A: switch `python3 -m spacehasten.remote.<X>` → script-path invocation (option A; user-confirmed); add `Settings.paths.spacehasten_src_dir` + `remote_script_path()` helper; update `install_spacehasten.py` to record it. Stage B: restructure `cli/verify.py` so each of the six checks owns its subdir+`.dbsh`; training check ingests `example.csv` directly via a verify-internal CSV-to-DB shim (option B.2.a; no public API change to `stages/training.train`). Stage C: auto-discover `spacehasten.ini` next to fixtures/install dir; bump `spaces_file_default` to `/data/programs/BiosolveIT/spaces_new/REALSpace_83bn_2025-09.space`. Stage D (optional): tail per-task SLURM `.err`/`.out` into stage-layer exception messages. Detailed instructions in [docs/SESSIONS.md §15.1](docs/SESSIONS.md). |
...
| L1 | Library screening — `library-build`/`library-screen` (post-cutover feature, [docs/plan-library-screening.md](docs/plan-library-screening.md)) | done | | New `stages/library_build.py` (Enamine `.cxsmiles`→chunked zstd Parquet store + `manifest.json`, resumable array build) + `remote/library_build.py` (RDKit canonicalization/reghash + PC descriptors, Enamine columns by default or `--recompute-props`). New `stages/library_screen.py` (property-filter-pushdown + chemprop-score array job over a library store, `top-n`/`score-cutoff`/seed-dock-percentile selection, dedup against `data.reghash`, `--dry-run`) + `remote/library_infer.py` (vectorized property filter, no RDKit, then chemprop D-MPNN scoring mirroring `remote/predict.py`). No schema change: survivors inserted via new `Database.insert_library_hit` (`pred_score`/`pred_version` set, `simsearch_cycle`/`dock_score` NULL — same no-schema-change trick as simsearch hits vs seeds). New `Database.seed_dock_score_percentile`/`filter_existing_reghashes` (the latter also now backs `stages/simsearch._existing_reghashes`, de-duplicating that logic). New `WorkDir.library_screen_dir`, `Settings.general.{cpu_count_library,library_infer_*,library_default_top_pct,library_build_chunk_size}`, `Settings.paths.library_store_default`. CLI: `library-build` (Setup group) and `library-screen` (Manual stages group) subcommands in `cli/main.py`. `pyarrow>=14.0` added as a runtime dependency. 30+ new tests across `tests/unit/{test_db_library,test_library_manifest,test_library_build_worker,test_library_infer,test_library_screen_helpers,test_cli}.py` and `tests/integration/{test_library_build_local,test_library_screen_local}.py`; ruff clean; mypy clean on the `core/stages/scheduler/workspace` strict scope. |
...
| 2026-04-29 | 15 | Legacy tree moved to `legacy/` rather than deleted. | The session 15 spec keeps it for one release as a regression baseline; users can fall back to legacy commands while validating the new package on real workloads. |
| 2026-04-29 | 15 | `spacehasten-legacy-gui` shim uses `runpy.run_path` after `cd legacy/` and `sys.path.insert(0, legacy/)`. | Legacy modules use bare `import functions` style imports between siblings; treating `legacy/` as a Python package is impossible without rewriting all those imports, which we explicitly do not want to do for one-release-deprecated code. The shim accepts an explicit `SPACEHASTEN_LEGACY_DIR` env var override for non-development installs. |
| 2026-05 | L1 | Library-screen survivors distinguished from seeds/simsearch hits with no schema change: `pred_score`/`pred_version` set, `simsearch_cycle`/`dock_score` NULL. | Matches the existing simsearch-hit-vs-seed convention (simsearch hits already set `simsearch_cycle`); avoids an ALTER TABLE migration for `.dbsh` databases already in the field. |
| 2026-05 | L1 | `library-screen`'s property filter reuses the Enamine-precomputed PC columns (MW/sLogP/HBA/HBD/RotBonds/TPSA) by default instead of recomputing via RDKit per campaign. | RDKit recomputation at 50M-1B scale for every campaign is the dominant cost the vectorized-filter design is meant to avoid; `--recompute-props` on `library-build` is the escape hatch when parity with the RDKit-based `import-seeds`/`search` filters matters more than speed. |
