...
from .recovery import recover_docking_input_acquisitions
...
def analyze_run(
    ...
    artifacts = render_plots(analysis_root, config.dpi, config.hit_threshold)
    manifest = {
        ...
        "inputs": checksums(
            [
                ...
                *(path for _, path in context.acquisition_paths),
                *(path for _, paths in context.docking_input_paths for path in paths),
            ]
        ),
